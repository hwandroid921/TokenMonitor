...
> Windows에서 Codex, Claude, Antigravity 사용량과 quota 상태를 한 화면에서 확인하는 데스크톱 앱입니다.
...
- Codex, Claude, Antigravity quota 상태를 provider card로 표시
...
대시보드는 Codex, Claude, Antigravity 카드를 보여줍니다. 각 카드는 provider별 플랜, quota, reset 상태를 표시하고, 연동이 필요한 경우 카드 안에서 필요한 조치를 안내합니다.
...
8. 창을 닫으면 설정에 따라 시스템 트레이로 최소화되거나 종료 확인 창이 표시됩니다.
