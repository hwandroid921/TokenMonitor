...
Token Monitor는 여러 AI 서비스의 로컬 로그인 상태와 제공자가 제공하는 사용량 정보를 읽어, 플랜·사용량·잔여량·초기화 시간을 대시보드와 화면 우측 하단 오버레이로 표시합니다. 지원되는 요금제 조건을 충족한 ChatGPT, Claude, Antigravity 사용자를 대상으로 합니다.

**지원 환경:** Windows x64, macOS Apple Silicon
**배포 형식:** Windows 포터블 실행 파일, macOS ARM64 DMG/ZIP

[릴리즈 다운로드](https://github.com/hwandroid921/TokenMonitor/releases)

## 핵심 기능

- Codex, Claude, Antigravity의 상태를 분리해 오버레이로 표시
- 공급자별 주간·5시간 사용량의 잔여량, 초기화까지 남은 시간, 초기화 예정 시각을 표시
- 계정별 별칭을 지정 및 관리하여 대시보드와 오버레이에 표시해서 어떤 계정인지 빠르게 구분 가능
...
| 항목                       | 필요 대상               | 설명                                            |
| ------------------------ | ------------------- | --------------------------------------------- |
| Node.js LTS 및 npm        | Claude, Antigravity | 앱 안의 CLI 로그인과 사용량 수집에 필요합니다.                  |
| Codex Desktop 설치 및 로그인   | ChatGPT/Codex       | 로컬 ChatGPT 사용량 확인에 필요합니다.                     |
| Claude Code 설치 및 로그인     | Claude              | Claude Code Status Line으로 quota를 표시할 때 필요합니다. |
| Antigravity 설치 및 CLI 로그인 | Antigravity         | CLI 로그인 수집 또는 실행 중인 앱의 로컬 수집 경로 중 하나가 필요합니다.  |


## 유저 사용 흐름

1. Token Monitor를 실행하고 대시보드의 새로고침 버튼을 누릅니다.
2. ChatGPT는 **Codex 연결 설정**에서 실행 파일을 확인하거나 지정한 뒤 Codex Desktop 로그인 상태를 확인합니다.
3. Claude는 **Claude 로그인**으로 OAuth 인증을 시작합니다. 앱은 Status Line 자동 등록 결과를 표시하며, 필요하면 **Status Line 등록** 또는 **재등록** 버튼으로 다시 설정할 수 있습니다. 마지막으로 열려 있던 Claude Code를 종료하고 새 터미널에서 `claude`를 실행해 일반 대화의 첫 응답을 받으세요. 첫 수집 전에는 대시보드와 오버레이에 안내가 표시됩니다.
4. Antigravity는 **Antigravity CLI 로그인**을 눌러 Google 인증을 완료합니다.
5. 설정 탭에서 감지된 계정의 마스킹 이메일을 확인하고 계정 별 별칭을 지정합니다.
6. 오버레이와 제공자 별 표시 항목을 조정합니다.
7. 사용량 알림을 켜고 5% 단위의 잔여량 임계치, Windows 알림, 전면 경고, 오버레이 색상 강조를 선택합니다.
...
**계정 및 별칭 관리**에서는 ChatGPT, Claude, Antigravity Google 계정을 관리합니다. 현재 계정 감지 여부와 마스킹 이메일, 사용자가 지정한 표시 이름/별칭, 감지 방식을 확인할 수 있습니다. 현재 로그인 계정의 등록을 삭제해도 공급자 로그인이나 credential은 삭제되지 않습니다.

**사용량 알림**에서는 ChatGPT, Claude, Antigravity의 자동 알림 대상을 선택하고 잔여량 5%부터 50%까지 5% 단위 임계치를 복수로 지정할 수 있습니다. 앱이 실행 중이면 Electron 메인 프로세스가 1분마다 사용량을 수집합니다. 잔여량이 선택한 임계치를 실제로 통과할 때 한 번 알리고, 모두 소진되거나 공급자의 다음 quota 주기가 실제로 확인되었을 때 별도 알림을 보냅니다.
...
초기화 예정 시간이 확인되면 해당 시각 직후 사용량을 다시 수집합니다. 공급자의 reset 시각이 다음 주기로 이동하고 잔여량이 실제로 회복된 경우에만 초기화 알림을 보냅니다. 절전이나 공급자 반영 지연이 있으면 제한적으로 다시 확인합니다. Claude는 최근 Claude Code Status Line 스냅샷이 있어야 합니다.

앱은 사용자 데이터 폴더에 오버레이·알림 설정, 중복 알림 방지용 표시 안전 quota 상태, 운영체제 보안 저장소로 암호화한 계정 별칭 매핑, Claude Status Line의 최소 스냅샷만 저장합니다.
...
| 상태 | 필요한 조치 |
| --- | --- |
| Node.js/npm을 찾을 수 없음 | **Node.js 설치 안내**를 열어 Node.js LTS와 npm을 설치합니다. |
| Claude CLI 로그인 필요 또는 로그인 확인 실패 | **Claude 로그인** 버튼으로 Claude.ai OAuth 로그인을 완료한 뒤 새로고침합니다. |
| Status Line 미등록 또는 설정 오류 | **Status Line 등록** 또는 **재등록**을 실행합니다. 설정 파일 오류라면 권한·형식을 확인합니다. |
