...
Token Monitor는 여러 AI 서비스의 로컬 로그인 상태와 제공자가 제공하는 사용량 정보를 읽어, 플랜·사용량·잔여량·초기화 시간을 대시보드와 화면 우측 하단 오버레이로 표시합니다. 해당 프로그램은 ChatGPT, Claude, Antigravity 사용자를 대상으로 합니다.

**지원 환경:** Windows x64, macOS Apple Silicon · **현재 앱 버전:** `v1.6.0` · **배포 형식:** Windows 포터블 실행 파일, macOS ARM64 DMG/ZIP

[릴리즈 다운로드](https://github.com/hwandroid921/TokenMonitor/releases)

## 핵심 기능

- ChatGPT/Codex, Claude, Antigravity의 상태를 분리해 오버레이로 표시
- 접을 수 있는 좌측 메뉴와 공급자별 세로 레인을 사용해 모델·별칭·플랜·수집 상태를 한 화면에서 비교
- 공급자별 주간·5시간 사용량의 잔여량, 초기화까지 남은 시간, 초기화 예정 시각을 같은 강조 수준으로 표시
- 주간 사용량과 5시간 사용량 표시
...
Windows에서는 `scripts\build-package-windows.bat`를 실행하면 현재 `package.json`의 버전을 기준으로 타입 검사, 프로덕션 빌드, 포터블 패키징, SHA-256 출력, 이전 포터블 실행 파일 정리를 순서대로 진행합니다. Node.js LTS와 npm이 설치되어 있어야 하며, 이미 실행 중인 패키징 작업이 있으면 중단합니다.
...
3. Claude는 연결 목록의 네 단계를 순서대로 확인합니다. **Node.js/npm**이 없으면 설치 안내가 표시됩니다. 준비가 끝나면 **Claude 로그인**으로 OAuth 인증을 시작합니다. 앱은 Status Line 자동 등록 결과를 표시하며, 필요하면 **Status Line 등록** 또는 **재등록** 버튼으로 다시 설정할 수 있습니다. 마지막으로 열려 있던 Claude Code를 종료하고 새 터미널에서 `claude`를 실행해 일반 대화의 첫 응답을 받으세요. 첫 수집 전에는 대시보드와 오버레이에 안내가 표시됩니다.
4. Antigravity는 **Antigravity CLI 로그인**을 눌러 Google 인증을 완료합니다. Antigravity가 이미 실행 중이면 로컬 fallback도 시도합니다.
5. 설정 탭에서 감지된 Antigravity Google 계정의 마스킹 이메일을 확인하고 계정별 별칭을 지정합니다.
6. 오버레이와 제공자별 표시 항목을 조정합니다.
...
설정의 **계정 및 별칭 관리**에서는 ChatGPT, Claude, Antigravity Google 계정을 관리합니다. 현재 계정 감지 여부와 마스킹 이메일, 사용자가 지정한 표시 이름/별칭, 감지 방식을 확인할 수 있습니다. 현재 로그인 계정의 등록을 삭제해도 공급자 로그인이나 credential은 삭제되지 않습니다.

설정의 **사용량 알림**에서는 ChatGPT, Claude, Antigravity의 자동 알림 대상을 선택하고 잔여량 5%부터 50%까지 5% 단위 임계치를 복수로 지정할 수 있습니다. 앱이 실행 중이면 Electron 메인 프로세스가 1분마다 사용량을 수집합니다. 잔여량이 선택한 임계치를 실제로 통과할 때 한 번 알리고, 모두 소진되거나 공급자의 다음 quota 주기가 실제로 확인되었을 때 별도 알림을 보냅니다.
