...
오버레이는 기본 디스플레이의 우측 하단에 표시되는 클릭 통과형 창입니다. 텍스트는 50% 투명도·검은 외곽선으로 표시되며, 내용에 맞춰 화면 높이의 1/3을 넘지 않도록 자동 조절됩니다. 표시 항목을 줄여도 텍스트는 창의 하단에 정렬됩니다. 설정의 **오버레이 위치**에서 위치 조정 모드를 시작하면 오버레이를 원하는 화면·위치로 드래그할 수 있으며, 오버레이의 완료 버튼을 누르면 클릭 통과 상태로 돌아갑니다. 기본 위치 복원도 같은 설정에서 할 수 있습니다.
