import { app, BrowserWindow, Menu, Notification, Tray, dialog, ipcMain, nativeImage, powerMonitor, screen, shell, session } from "electron";
...
function removeGeminiAppsIntegrationData() {
  const userData = app.getPath("userData");
  for (const filename of ["gemini-apps-usage.json", "gemini-apps-session.json"]) {
    try {
      fs.rmSync(path.join(userData, filename), { force: true });
    } catch {
      // The deprecated Gemini Apps cache is optional and may already be absent.
    }
  }
  void session.fromPartition("persist:gemini-usage").clearStorageData().catch(() => undefined);
}
...
if (!gotSingleInstanceLock) {
  ...
  app.whenReady().then(() => {
    ...
    removeGeminiAppsIntegrationData();
    ...
    ipcMain.handle("developer-diagnostics:read", (event) => isMainWindowSender(event) ? readDeveloperDiagnostics() : { enabled: false, generatedAt: new Date().toISOString(), environment: getDeveloperEnvStatus(), providers: [] });
